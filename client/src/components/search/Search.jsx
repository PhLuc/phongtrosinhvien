...
const Search = ({ dispatch, navigate, location }) => {
  ...
  const handleSearchPost = () => {
    ...
    if (priceRange) queries.price = priceRange.value?.filter((el) => el !== null)
  ...
  return (
    <div className="my-4 w-main mx-auto bg-main-yellow p-2 rounded-md gap-2 grid grid-cols-5">
      ...
      <div
        onClick={() =>
          dispatch(
            modal({
              isShowModal: true,
              modalContent: <SearchAddress getAddress={(val) => setValue("address", val)} />,
            })
          )
        }
      ...
      >
        <span className="line-clamp-1">{priceRange?.text || "💲 Giá thuê"}</span>
      </div>
      <div
        className="col-span-1 p-2 rounded-[0.25rem] flex items-center cursor-pointer text-gray-500 bg-white"
        onClick={() =>
      ...
      >
        <span className="line-clamp-1">{areaRange?.text || "🔳 Diện tích"}</span>
      </div>
