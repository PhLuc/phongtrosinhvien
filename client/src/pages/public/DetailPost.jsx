...
import { Button, Comments, Map, Rating, RelatedPost, Slider, TypeBox } from "@/components"
...
const DetailPost = ({ navigate, location }) => {
  ...
  const fetchPostById = async () => {
    ...
    if (news.success) setRelated((prev) => ({ ...prev, news: news.posts?.rows }))
    ...
    <section className="mb-6">
      {!post?.isAvailable && (
        <p className="p-4 border rounded-md border-main-pink text-main-pink bg-transparent text-center w-full inline-block mt-6 text-sm">
          Chỗ nghỉ này đã được người cho thuê đánh dấu là ĐÃ ĐƯỢC THUÊ. Bạn có thể liên hệ người cho thuê để thông tin
          thêm nêu cần.
        </p>
      )}
      ...
      <div className="w-full grid grid-cols-10 gap-4">
        <div className="col-span-7 bg-white rounded-md border">
          <Slider count={1}>
            {post?.images?.map((el) => (
              <img key={el} src={el} alt="avatar" className="h-[350px] object-contain m-auto" />
            ))}
          </Slider>
          <div className="p-4 text-sm flex flex-col gap-3">
            ...
            <span>
              ...
              <span className="text-main-blue font-semibold underline cursor-pointer">
                {post?.cates?.value + " " + post?.address?.split(",")?.slice(-1)?.pop()}
              </span>
            </span>
            ...
            <div className="grid grid-cols-3">
              <span className="flex items-center gap-2">
                💰
                <span className="text-lg font-bold text-green-600">{customMoney(post?.price)}</span>
              </span>
              <span className="flex items-center gap-2">
                🔳<span>{post?.area}</span>
                <span>
            ...
            <div className="mt-6">
              ...
              <div className="grid grid-cols-10">
                ...
                <div className="col-span-7 flex flex-col">
                  ...
                  <span className="p-2  border-[0.5px] bg-gray-100">{post?.cates?.value}</span>
            ...
            <div className="mt-6">
              ...
              <div className="grid grid-cols-10">
                ...
                <div className="col-span-7 flex flex-col">
                  <span className="p-2  border-[0.5px]">{post?.author?.name}</span>
                  <span className="p-2  border-[0.5px] bg-gray-100">{post?.author?.phone}</span>
                  <span className="p-2  border-[0.5px]">{post?.author?.phone}</span>
                </div>
              </div>
            </div>
            <div className="mt-6">
              ...
              <div className="w-full h-[250px]">
                <Map address={post?.address?.replace("Địa chỉ:", "")} zoom={16} />
              </div>
            </div>
          </div>
        </div>
        <div className="col-span-3 flex flex-col gap-4">
          <div className="rounded-md bg-main-yellow p-4 flex gap-2 flex-col justify-center items-center">
            ...
            <span className="flex fap-2 items-center text-sm">
              <GoDotFill size={20} color={post?.author?.isBlocked ? "red" : "green"} />
              <span>{post?.author?.isBlocked ? "Tài khoản tạm khóa" : "Đang hoạt động"}</span>
            </span>
            ...
            <Button className="bg-white text-main-blue border border-blue-600" fullWidth>
      ...
      <div className="mt-6">
        <Rating name={post?.title} votes={post?.votes} star={post?.star} pid={post?.id} />
      </div>
      <div className="w-full mt-4 p-4  rounded-md bg-white drop-shadow-sm">
        <h1 className="text-xl font-bold tracking-tight mb-6">Trao đổi và bình luận</h1>
