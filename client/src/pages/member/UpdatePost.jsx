...
const UpdatePost = ({ dispatch, editPost, render }) => {
  ...
  const setCustomValue = (id, val) => setValue(id, val, { shouldDirty: true, shouldTouch: true, shouldValidate: true })
  useEffect(() => {
  ...
  // Tạo địa chỉ từ province / district / ward + gọi lấy tạo độ vị trí địa chỉ đó
  useEffect(() => {
    ...
    // if (textModified) fetLngLat({ text: textModified, apiKey: import.meta.env.VITE_MAP_API_KEY })
    ...
    >
      ...
      <section className="p-4 grid grid-cols-12 gap-6">
        <div className="col-span-8">
          ...
          <div className="mt-6">
            <InputFile getFile={(val) => setCustomValue("images", val)} label="Hình ảnh" image={images} id="images" />
          </div>
        </div>
        <div className="col-span-4 flex flex-col gap-4">
          <div className="w-full relative z-10 h-[300px]"> <Map address={address} zoom={zoom} /></div>
          <div className="flex flex-col p-4 text-sm gap-2 rounded-md bg-orange-100 border border-orange-500 text-orange-600">
            ...
            <ul className="list-item pl-8">
              ...
              <li className="list-disc">Các bạn nên điền đầy đủ thông tin vào các mục để tin đăng có hiệu quả hơn.</li>
