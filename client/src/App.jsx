...
import { LayoutAdmin, Dashboard, ManageUser, ManagePosts, ManageExpired } from "./pages/admin"
import { LayoutMember, Personal, CreatePost, ManagePost, Wishlist } from "./pages/member"
import { useSelector, useDispatch } from "react-redux"
import { useEffect } from "react"
import { getCategories, getCurrent, getDataVn, getOptions } from "./redux/actions"
import { apiUpdateViews } from "./apis/app"

function App() {
  const dispatch = useDispatch()
  const { isLoading, categories, isShowModal, modalContent } = useSelector((state) => state.app)
      ...
      <Routes>
        <Route path={path.LAYOUT} element={<Layout />}>
          ...
          <Route
            ...
            element={<Chothuecanho {...categories?.find((el) => el.slug === path.CHOTHUECANHO)} />}
          />
          <Route
            path={path.CHOTHUEPHONGTRO}
            element={<Chothuephongtro {...categories?.find((el) => el.slug === path.CHOTHUEPHONGTRO)} />}
          />
          <Route
            path={path.NHACHOTHUE}
            element={<Nhachothue {...categories?.find((el) => el.slug === path.NHACHOTHUE)} />}
          />
          <Route path={path.TIMOGHEP} element={<Timoghep {...categories?.find((el) => el.slug === path.TIMOGHEP)} />} />
        </Route>
