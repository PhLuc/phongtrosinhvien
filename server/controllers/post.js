...
const getPosts = asyncHandler(async (req, res) => {
  const { limit, page, fields, order, address, price, area, keyword, isPublic, ...query } = req.query
  const limitQuery = +limit || +process.env.LIMIT
  const offset = +page - 1 || 0
  const offsetQuery = offset * limitQuery
  if (keyword) query[Op.or] = [{ title: { [Op.iLike]: `%${keyword}%` } }, { address: { [Op.iLike]: `%${keyword}%` } }]
...
const updatePost = asyncHandler(async (req, res) => {
  ...
  return res.json({
    ...
    mes: response[0] > 0 ? "Cập nhật tin đăng thành công" : "Có lỗi, hãy thử lại sau.",
  })
})
...
const createRequestRequired = asyncHandler(async (req, res) => {
  ...
  if (alreadyExpiredPost) {
    await db.Expired.update({ ...req.body, uid }, { where: { id: alreadyExpiredPost.id } })
  } else {
    await db.Expired.create({ ...req.body, uid })
  }
  if (req.body?.status === "Accepted") {
    await db.Post.update({ expiredDate: Date.now() + days * 24 * 3600 * 1000 }, { where: { id: pid } })
  }
  return res.json({
    success: true,
...
const updateExpired = asyncHandler(async (req, res) => {
  ...
  if (status === "Accepted") {
    const [post, expired] = await Promise.all([
      db.Post.update({ expiredDate: Date.now() + days * 24 * 3600 * 1000 }, { where: { id: pid } }),
    ...
    const response = await db.Expired.update({ status }, { where: { id: expiredId } })
    return res.json({
      success: response[0] > 0 ? true : false,
